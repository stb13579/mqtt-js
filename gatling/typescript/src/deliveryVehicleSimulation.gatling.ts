...
type LoadProfile = "ramp" | "spike" | "steady" | "soak" | "smoke";

type OpenInjectionStep = ReturnType<typeof atOnceUsers>;
type ProfileDefaults = {
  deviceCount: number;
  durationSeconds: number;
  steps: (count: number) => OpenInjectionStep[];
};

export default simulation((setUp) => {
  const requestedProfile = getParameter("loadProfile", "ramp").toLowerCase();
  const isKnownProfile = (value: string): value is LoadProfile =>
    ("ramp spike steady soak smoke".split(" ") as LoadProfile[]).includes(value as LoadProfile);
  const profileKey: LoadProfile = isKnownProfile(requestedProfile) ? requestedProfile : "ramp";
  const telemetryIntervalMs = Math.max(50, parseInt(getParameter("telemetryIntervalMs", "1000"), 10));

  const profileCatalog: Record<LoadProfile, ProfileDefaults> = {
    ramp: {
      deviceCount: 5000,
  ...
  const updateInterval = { amount: telemetryIntervalMs, unit: "milliseconds" } as const;
  ...
  const nextOffset = (radius: number) => (Math.random() - 0.5) * 2 * radius;
  ...
  const scn = scenario(`Telemetry-${profileKey}`)
    ...
    .on(
      exec((session) => {
        const currentLat = session.get<number>("lat");
        const currentLng = session.get<number>("lng");
        const currentFuel = session.get<number>("fuelLevel");
